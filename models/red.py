...
class RED(BaseModel):
    ...
    #     model_opt = dataset_name  + "-" + date + "-" + opt.model + str(opt.n_layers)
...
class REDModel(nn.Module):
    def __init__(self, opt):
        ...
        conv_layers.append(nn.Sequential(nn.Conv2d(n_channels, num_features, kernel_size=3, stride=2, padding=1),
                                         nn.ReLU(inplace=True)))
        for i in range(num_layers - 1):
            conv_layers.append(nn.Sequential(nn.Conv2d(num_features, num_features, kernel_size=3, padding=1),
                                             nn.ReLU(inplace=True)))
        ...
        for i in range(num_layers - 1):
            deconv_layers.append(nn.Sequential(nn.ConvTranspose2d(num_features, num_features, kernel_size=3, padding=1),
                                               nn.ReLU(inplace=True)))
        deconv_layers.append(nn.ConvTranspose2d(num_features, n_channels, kernel_size=3, stride=2, padding=1, output_padding=1))

        # self.sub_mean = common.MeanShift(1.0, n_channels=opt.n_channels)

        self.conv_layers = nn.Sequential(*conv_layers)
        self.deconv_layers = nn.Sequential(*deconv_layers)
        self.relu = nn.ReLU(inplace=True)

        # self.add_mean = common.MeanShift(1.0, n_channels=opt.n_channels, sign=1)
